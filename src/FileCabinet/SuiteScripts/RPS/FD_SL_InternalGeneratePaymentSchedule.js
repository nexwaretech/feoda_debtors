...
let week_post = ["first", "second", "third", "fourth", "last"];
let ACTIVE = 2;
...
define([
  ...
  function onRequest(context) {
    let LOG_TITLE = "onRequest:rps_internalpaysched";
  ...
  function renderForm(context) {
    ...
    let tranid = context.request.parameters[lib.SCRIPTS.sl_gen_sched.params.id];
    ...
    let debtorbalance = debtor_dtl.getValue("balance"); //Kel 02052018
    ...
    let contactDetails = lib.searchDebtorContacts({
      debtor: trn_fld["entity"][0]["value"],
    });

    let renderer = render.create();
    renderer.templateContent = file
      .load(lib.FOLDER + lib.FILE.rps_internal)
      .getContents();
