  ...
  function onRequest(context) {
    ...
    if (context.request.method === "GET") {
      try {
        ...
        let images = lib_files.searchFileUrlinFolder("images_shared");
        ...
        const paymentMethods = [
          {
            id: "4",
            name: "Master Card",
            url: images.ccmastercardpng,
          },
          {
            id: "5",
            name: "VISA",
            url: images.ccvisapng,
          },
        ];
          ...
          data: JSON.stringify({
            tempData: {
              selectedPayments: JSON.stringify(
                bpRec.getValue(
                  lib_billing_preference.REC_BILLING_PREFERENCE.PYMT_METHOD
                )
              ),
              selectedCardOptions: JSON.stringify(
                bpRec.getValue(
                  lib_billing_preference.REC_BILLING_PREFERENCE.CARD_OPTIONS
                )
              ),
              selectedSchedule: JSON.stringify(
                bpRec.getValue(
                  lib_billing_preference.REC_BILLING_PREFERENCE.PYMT_SCHEDULE
                )
              ),
              useExisting: bpRec.getValue(
                lib_billing_preference.REC_BILLING_PREFERENCE.USE_EXISTING
              ),
              isVoluntary: bpRec.getValue(
                lib_billing_preference.REC_BILLING_PREFERENCE.IS_VOLUNTARY
              ),
              volunAmt: bpRec.getValue(
                lib_billing_preference.REC_BILLING_PREFERENCE.VOLUN_AMT
              ),
              discPerc: bpRec.getValue(
                lib_billing_preference.REC_BILLING_PREFERENCE.DISC_PERC
              ),
              discItem: bpRec.getValue(
                lib_billing_preference.REC_BILLING_PREFERENCE.DISC_ITEM
              ),
              appliesTo: bpRec.getValue(
                lib_billing_preference.REC_BILLING_PREFERENCE.APPLIES_TO
              ),
            },
        ...
        renderer.templateContent = file
          .load("./files_billing/billing_config4.html")
          .getContents();
