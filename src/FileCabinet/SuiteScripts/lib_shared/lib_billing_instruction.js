 ...
 * Author: Feoda
 */
define(['N/search', 'N/record', './lib_const.js', './lib_billing_engine.js', './lib_billing_preference.js'], function ( search,record, lib_const, lib_billing_engine, lib_billing_preference,) {


    const REC_BILLING_INSTRUCTION = {
        ID : 'customrecord_xw_billing_inst',
        BILLING_TYPE: "custrecord_xw_binsttype",
        ITEM: "custrecord_xw_binstitem",
        YEAR: "custrecord_xw_binstyear",
        PRICE: "custrecord_xw_binstitemprice",
        FAMILY: "custrecord_xw_binst_family",
        FAMILY_ORDER: "custrecord_xw_binst_family_order"
    }


    function getInstructionListFromItems(rec) {

        var arrInstructions = [], arrCategories = [];

        var filterCharges = rec.getValue(lib_billing_preference.REC_BILLING_PREFERENCE.CHARGES);
        var filterDiscounts = rec.getValue(lib_billing_preference.REC_BILLING_PREFERENCE.DISCOUNTS);
        var filterBillingEngineType = filterCharges.concat(filterDiscounts);
        var filterItems = rec.getValue(lib_billing_preference.REC_BILLING_PREFERENCE.CHARGES);

        var filters = [];

        if (filterBillingEngineType.length > 0) {
            filters.push([REC_BILLING_INSTRUCTION.BILLING_TYPE, "anyof", filterBillingEngineType]);
        } else {
        ...
        searchObj.run().each(function (result) {
            var objInstruction = {};
            if (arrCategories.findIndex(category => category.id == result.getValue(REC_BILLING_INSTRUCTION.BILLING_TYPE)) == -1) {
                arrCategories.push({
        ...
        const sortedCategories = arrCategories.sort((a, b) => ((a.category > b.category) ? 1 : -1));
        return {
