...
define(['N/render', 'N/file', 'N/record',
  '../lib_shared/lib_billing_preference.js', '../lib_shared/lib_utils.js',
  '../lib_shared/lib_billing_instruction.js'], function (
    render,
    file,
    record,
  ...
  function onRequest(context) {
      ...
      try {
        ...
        if (context.request.method === 'GET') {
          ...
          var output = renderForm(bpRec, sHome);
  ...
  function renderForm(bpRec, sHome) {
    ...
    var template = file.load('./billing_config2.html').getContents();
    renderer.templateContent = template;

    var instructionObj = lib_billing_instruction.getInstructionListFromItems(bpRec);
    var arrInstructions = instructionObj.arrInstructions;
    var arrCategories = instructionObj.arrCategories;
    ...
    let tempData = {
      ...
      isDebtor: bpRec.getValue(lib_billing_preference.REC_BILLING_PREFERENCE.FAMILY_CODE),
